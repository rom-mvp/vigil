...
class VigilMetrics:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            cache_hit_rate = (self.cache_hits / total_cache * 100.0) if total_cache else 0.0
            return {
...
class AgentShieldClient:
    ...
    def enforce(self, enforcement_request: dict) -> dict:
        ...
        # Select endpoint based on presence of blind envelope (payload.ciphertext)
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if action == "ALLOW":
                    ...
                    self.metrics.update_cache_size(len(self.decision_cache))
    ...
    def get_decision(self, decision_request: dict) -> dict:
        ...
        Falls back to /enforce if /decision is not available.
        """
        ...
        try:
            ...
            if envelope.get("decision") == "ALLOW":
                self._cache_decision(input_hash, envelope)
                self.metrics.update_cache_size(len(self.decision_cache))
            return envelope
        except Exception as e:
            # Fallback to /enforce endpoint
            enforce_url = f"{self.base_url}/v1/enforce"
            try:
                resp = requests.post(
                    enforce_url,
                    json=decision_request,
                ...
                # Map /enforce response fields to /decision response fields for compatibility
                if "action" in envelope and "decision" not in envelope:
                    envelope["decision"] = envelope["action"]
                envelope.setdefault("input_hash", input_hash)
                
                # For /enforce responses, just return as-is (already has decision, risk_score, etc.)
                envelope["sig_verified"] = False  # /enforce doesn't provide signatures
                if envelope.get("decision") == "ALLOW":
                    self._cache_decision(input_hash, envelope)
                    self.metrics.update_cache_size(len(self.decision_cache))
                return envelope
    ...
    def get_llm_credentials(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Retrieve LLM API credentials from AgentShield Vault (SaaS mode).
        
        Args:
            request_data: Request context with tenant_id, agent_id, policy_id
            
        Returns:
            Dict with:
                - provider: string (openai, anthropic, etc.)
                - api_key: string (encrypted or plaintext depending on transport)
                - endpoint: optional custom endpoint URL
                - model: optional model override
        
        Flow:
            1. Vigil calls this after decision is ALLOW
            2. AgentShield loads tenant's vault entry
            3. Returns encrypted API key for the requested provider
            4. Vigil uses this key to forward to LLM
        """
        url = f"{self.base_url}/vault/credentials"
        ...
        payload = {
            ...
            "policy_id": request_data.get("policy_id"),
            "request_id": request_data.get("request_id"),
            "provider": request_data.get("provider", "openai"),  # Default to OpenAI
            "timestamp_ms": request_data.get("timestamp_ms") or int(time.time() * 1000)
        }
        
        try:
            # Call AgentShield vault endpoint
            resp = requests.post(
                url,
                json=payload,
                timeout=self.timeout_ms / 1000.0,
            ...
            if resp.status_code == 200:
                ...
                # {
                #   "provider": "openai",
                #   "api_key": "sk-...",  # Encrypted in transit via TLS
                ...
                return {
                    "provider": vault_data.get("provider", "openai"),
                    "api_key": vault_data.get("api_key"),
                    "endpoint": vault_data.get("endpoint"),
                    "model": vault_data.get("model"),
                    "expires_at": vault_data.get("expires_at")
                }
            
            elif resp.status_code == 404:
                # No credentials configured for this tenant
                error = RuntimeError(f"No LLM credentials configured for tenant {payload['tenant_id']}")
                error.vigil_error_code = "VAULT_CREDENTIALS_NOT_FOUND"
                raise error
            
            elif resp.status_code == 403:
                # Policy doesn't allow this request
                error = RuntimeError(f"Vault access denied for tenant {payload['tenant_id']}")
                error.vigil_error_code = "VAULT_ACCESS_DENIED"
                raise error
            
            else:
                error = RuntimeError(f"Vault credentials request failed: {resp.status_code}")
                error.vigil_error_code = "VAULT_REQUEST_FAILED"
                raise error
                
        except requests.Timeout as e:
            error = RuntimeError(f"Vault credentials request timed out")
            error.vigil_error_code = VigilErrorCode.AGENTSHIELD_TIMEOUT
            raise error from e
        
        except requests.RequestException as e:
            error = RuntimeError(f"Vault credentials request failed: {e}")
            error.vigil_error_code = VigilErrorCode.AGENTSHIELD_UNREACHABLE
            raise error from e
